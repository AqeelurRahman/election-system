<x-forms.patch :action="route('frontend.user.profile.update')">
    ...
    <div class="form-group row">
        <label for="name" class="col-md-3 col-form-label text-md-right">@lang('UVC')</label>
        <div class="col-md-9">
            <input type="text" name="uvc" class="form-control" placeholder="{{ __('UVC') }}" value="{{ old('name') ?? $logged_in_user->uvc }}" required autofocus autocomplete="uvc" />
        </div>
    </div>
